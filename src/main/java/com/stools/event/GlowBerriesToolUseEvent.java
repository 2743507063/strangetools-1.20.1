...
public class GlowBerriesToolUseEvent {
    ...
    public static void register() {
        UseItemCallback.EVENT.register((player, world, hand) -> {
            ...
            if (player.isSneaking() && stack.getItem() instanceof ToolItem toolItem) {
                if (toolItem.getMaterial() == ModToolMaterials.GLOW_BERRIES) {
                    int currentDamage = stack.getDamage();
                    int maxDamage = stack.getMaxDamage();
                    int remainingDurability = maxDamage - currentDamage;
                    int consumeAmount = Math.min(remainingDurability, 32);

                    if (!world.isClient()) {
                        stack.damage(consumeAmount, player, e -> e.sendToolBreakStatus(hand));

                        HungerManager hungerManager = player.getHungerManager();
                        hungerManager.add(2, 0.4f);

                        player.addStatusEffect(new StatusEffectInstance(StatusEffects.GLOWING, 100, 1));
